...
a.starts_at |> Compare.gte(a.ends_at)            # a.starts_at >= a.ends_at
column("s", "expires_at") |> Compare.gt(db_now)  # s.expires_at > now()
```
...
```jade
p <- from(patients)
c <- patients |> aliased("c") |> join((c) -> { p.id |> Compare.eq(c.parent_id) })
```
...
```jade
p <- from(patients)
a <- left_join(appointments, (a) -> { p.id |> Compare.eq(a.patient_id) })
...
```jade
p.id |> nullable |> Compare.eq(a.patient_id)  # Expr(Int) → Expr(Maybe(Int))
```
