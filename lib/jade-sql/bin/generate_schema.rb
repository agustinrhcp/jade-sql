...
module JadeSql
  module SchemaGenerator
    ...
    TYPE_MAP = {
      ...
      # Extension and network types that arrive as their text form on the
      # `exec_query` path the runtime uses, so `String` is what they decode as
      # rather than an approximation of one. `citext` is text that compares
      # case-insensitively, which is a property of the column and not of the
      # value Jade holds.
      /\Acitext\b/ => "String",
      /\Ainet\b/ => "String",
      /\Acidr\b/ => "String",
    ...
    # the second of which is two words where one `\w+` would fit.
    IDENTITY = /\bGENERATED\s+(?:ALWAYS|BY\s+DEFAULT)\s+AS\s+IDENTITY\b/i

    # A generated column is computed by the database from the other columns.
    # Writing one is an error rather than merely redundant, so it is not
    # required and never can be.
    ...
    # `character varying(255)` and `character varying` are the same type as far
    # as Jade is concerned, and only the length stands between the second
    # spelling and the pattern that matches it. It matters most for arrays,
    # whose patterns end in `[]` and so cannot tolerate anything before it:
    # `character varying(255)[]` would otherwise type as a bare String and
    # fail at decode.
    ...
    # An extension type is written with the schema that owns it, the same way
    # an enum is: `public.citext`, `public.hstore`.
