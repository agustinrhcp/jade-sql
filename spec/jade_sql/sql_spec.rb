...
module Jade
  describe 'Sql (extension)' do
      ...
      it 'render >, >=, <, <= against the DB clock and params' do
        test_compiler.require('app', <<~JADE)
          ...
          def after_now -> Expr(Bool)
            column("s", "expires_at") |> Expr.gt(db_now)
          end
    ...
    describe 'Sql.Expr, against something already built' do
      it 'renders the same operators with an expression on the right' do
        test_compiler.require('app', <<~JADE)
          ...
          def against_column -> Expr(Bool)
            column("p", "id") |> Expr.eq(column("o", "person_id"))
          end
    ...
    describe 'within, for a column against a range' do
      # A distinct module per case: Ruby will not re-require a file it has
      # already loaded, so reusing one name would hand back the first render.
      def rendered(name, range)
        test_compiler.require(name, <<~JADE)
          module #{name.capitalize} exposing (p)

          import Sql exposing (Expr, column, within)
...
import Sql.Expr as Expr
import Sql.Query exposing (Query, Select, field, filter, from, join, select, to_sql)
...
def parents_and_kids -> Query(PersonsCols)
  ...
  persons
    |> aliased("c")
    |> join((c) -> { p.id |> Expr.eq(c.parent_id) })
end
        JADE
      end
...
#{jade_table('orders', { id: 'Int', person_id: 'Int' }, alias_: 'o', pk: 'orders_pk')}
...
def persons_with_optional_orders -> Query(OrdersLeftCols)
  p <- from(persons)
  left_join(orders, (o) -> { p.id |> Expr.eq(o.person_id) })
end
        JADE
      end
...
#{jade_table('companies', { id: 'Int' }, alias_: 'c', pk: 'companies_pk')}
...
def persons_with_companies -> Query(CompaniesCols)
  p <- from(persons)
  join(companies, (c) -> { p.company_id |> Expr.eq(c.id |> nullable) })
end
        JADE
      end
...
import Sql.Expr as Expr
import Encode
import Sql.Query exposing (Query, Select, field, from, join, select, to_sql, where)
import Decode exposing (Value)
...
#{jade_table(
  'orders',
  { id: 'Int', person_id: 'Int', total: 'Int' },
  alias_: 'o',
  pk: 'orders_pk',
)}
...
def query -> Select(Row)
  p <- from(persons)
  o <- join(orders, (o) -> { p.id |> Expr.eq(o.person_id) })
  select(Row(_, _, _))
    |> field(p.id)
    |> field(p.name)
    |> field(o.total)
...
def sorted_asc_q -> Select(Person)
  p <- from(persons)
  select(Person(_, _, _))
    |> field(p.id)
    |> field(p.name)
...
def sorted_desc_q -> Select(Person)
  p <- from(persons)
  select(Person(_, _, _))
    |> field(p.id)
    |> field(p.name)
...
def multi_sorted_q -> Select(Person)
  p <- from(persons)
  select(Person(_, _, _))
    |> field(p.id)
    |> field(p.name)
...
#{jade_table(
  'patients',
  { id: 'Int', name: 'String', balance: 'Int', archived: 'Bool' },
  alias_: 'p',
  pk: 'patients_pk',
)}
...
#{jade_table(
  'memberships',
  { user_id: 'Int', group_id: 'Int', role: 'String' },
  key: '(Int, Int)',
  pk: 'memberships_pk',
)}
